...
4. **Audio Manager.cs** - Singleton for music and SFX management
   ...
   - Universal Menu BGM for Main Menu and Level Select
   - Separate Chess Mode and Standoff Mode BGM per level
...
6. **Pawn Health.cs** - Unified health system (merged PlayerPawn + OpponentPawn)
   ...
   - Player always has 3 MaxHP and starts with 2 HP (enforced in code)
...
1. **Standoff Mode Trigger Fix**: Standoff mode now correctly triggers only when 1 opponent remains, not at game start
   - Added delay check and opponent count validation in `Game Manager.cs`
   - Prevents premature standoff transition during level initialization

2. **Player HP Configuration**: Player HP is now hardcoded to ensure consistency
   - Player always has 3 MaxHP and starts with 2 HP
   - Enforced in `PawnHealth.cs` Awake() and OnEnable()

3. **Tag-Based Game Logic**: Game logic now relies on tags instead of layers
   - Layers only used for Physics raycasts (ground detection, line-of-sight)
   - Tags used for gameplay identification (Player, Tile, Wall, Obstacle)
   - Improves code clarity and Unity best practices

### UI/UX Enhancements
4. **Automatic UI Panel Management**: UI panels now automatically activate/deactivate
   - Handled by `UI Manager.cs` based on game state changes
   - Mobile controls automatically shown/hidden in Standoff mode
   - Clean panel transitions with `InitialisePanelStates()` in Awake()

5. **Level Button Rendering**: Level select UI now renders buttons from centre outward
   - Centre level button appears 1.2x larger for visual emphasis
   - Level count retrieved from `GameManager.TotalLevels`
   - Alternating left/right rendering for balanced layout

### Audio System Overhaul
6. **Stage-Based BGM System**: Music now changes based on game state
   - **Menu BGM**: Universal track for Main Menu and Level Select
   - **Chess Mode BGM**: Separate track per level (from `LevelData.ChessModeMusic`)
   - **Standoff Mode BGM**: Separate track per level (from `LevelData.StandoffModeMusic`)
   - Automatic music switching handled by `GameManager.HandleStateTransition()`
   - Prevents music restart if already playing the same clip

### Code Cleanup
7. **Removed Unused Functions**: Cleaned up codebase
   - Removed `OnLevelSelectClicked()` from UI Manager (duplicate of OnPlayClicked)
   - Removed obsolete `BackgroundMusic` field from Level Data (replaced with mode-specific tracks)
...
*Player HP: 3 MaxHP, 2 StartHP (hardcoded)*
